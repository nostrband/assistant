...
export const webSearchTool = createTool({
  ...
  inputSchema: z.object({
    ...
    type: z
      ...
      .optional()
      .default("auto")
      .describe("Search type: 'neural' for semantic search, 'keyword' for exact matches, 'auto' for best results"),
    numResults: z
      .number()
      .int()
      .min(1)
    ...
    includeDomains: z
      .array(z.string())
      .optional()
      .describe("Array of domains to include in search (e.g., ['reddit.com', 'stackoverflow.com'])"),
    excludeDomains: z
      .array(z.string())
      .optional()
    ...
    startPublishedDate: z
      .string()
      .optional()
      .describe("Start date for content published date filter (ISO format: YYYY-MM-DD)"),
    endPublishedDate: z
      .string()
      .optional()
  ...
  execute: async ({ context }) => {
    ...
    try {
      ...
      const formattedResults = result.results.map((item: any, index: number) => ({
        rank: index + 1,
        title: item.title || "",
        url: item.url || "",
        ...
        score: item.score || 0,
        text: item.text ? item.text.substring(0, 2000) + (item.text.length > 2000 ? "..." : "") : null,
        summary: item.text ? item.text.substring(0, 300) + (item.text.length > 300 ? "..." : "") : null,
      }));
