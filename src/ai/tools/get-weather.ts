...
export const getWeatherTool = tool({
  description: "Get weather forecast for a specified location for up to 16 days. If location not found, try higher-level location name.",
  ...
  execute: async (context) => {
    ...
    try {
      ...
      const { latitude, longitude, timezone, name, country, admin1 } = geo.results[0];
      ...
      if (!wxRes.ok) throw new Error(`Forecast failed: ${wxRes.status} ${wxRes.statusText}`);
      ...
      const codeText = (code: number) => ({
        ...
        95: "Thunderstorm", 96: "Thunderstorm w/ hail (light)", 99: "Thunderstorm w/ hail (heavy)"
      })[code] ?? `Code ${code}`;
