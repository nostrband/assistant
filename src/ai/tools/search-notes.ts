...
import { searchNotes } from "@/lib/server/note-store";
import { tool } from "ai";
  ...
  execute: async (context) => {
    ...
    try {
      ...
      if (!keywords?.length && !tags?.length && !regexp) {
        return {
          success: false,
          error: "At least one search criteria must be provided (keywords, tags, or regexp)",
          notes: [],
          total_count: 0,
        };
      }
      ...
      const notes = await searchNotes(USER_ID, query);
